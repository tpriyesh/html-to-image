...
function initializeOptions(node, options) {
    const initialized = Object.assign({}, options);
...
function checkDOMComplexity(node, options) {
    if (options.maxNodes) {
        ...
        if (nodeCount > options.maxNodes) {
            throw new CaptureAbortedError(`Capture aborted: DOM has ${nodeCount} nodes, exceeds maximum of ${options.maxNodes}`, 'max_nodes');
        }
    }
}
export async function toSvg(node, options = {}) {
    ...
    const clonedNode = (await cloneNode(node, opts, true));
