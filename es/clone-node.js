...
import { createImage, toArray, isInstanceOfElement, getStyleProperties, yieldToMain, } from './util';
...
function checkLimits(options) {
    ...
    if (options.maxNodes && context && context.nodeCount >= options.maxNodes) {
        throw new CaptureAbortedError(`Capture aborted: exceeded maximum node limit of ${options.maxNodes}`, 'max_nodes');
    }
    // Check timeout
    if (options.timeout && options._startTime) {
        const elapsed = Date.now() - options._startTime;
        if (elapsed >= options.timeout) {
            throw new CaptureAbortedError(`Capture aborted: exceeded timeout of ${options.timeout}ms`, 'timeout');
        }
    }
}
/**
 ...
 */
async function cloneCanvasElement(canvas, options) {
 ...
 */
async function canvasToDataURLChunked(sourceCanvas, options) {
    ...
    for (let i = 0; i < totalStrips; i += 1) {
        ...
        if ((options === null || options === void 0 ? void 0 : options.nonBlocking) && i % 4 === 3) {
